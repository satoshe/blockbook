...
func (w *Worker) getEthereumContractBalance(index int, c *db.AddrContract, details AccountDetails) (*Token, error) {
	...
	if ca != nil {
		...
		if details > AccountDetailsBasic {
			...
			for i := range ca.Contracts {
				...
				t, err := w.getEthereumContractBalance(i+db.ContractIndexOffset, c, details)
				if err != nil {
